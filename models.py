# models.py: จัดการกับโครงสร้างข้อมูล (data models) และการติดต่อฐานข้อมูล SQLite
#
# คลาสต่างๆ เช่น Menu, Reservation, Bill ฯลฯ ใช้เก็บข้อมูลที่เกี่ยวข้องกับเมนู,
# การจอง, บิล โดยใช้ @dataclass เพื่อให้เขียนโค้ดได้กระชับ
#
# DatabaseManager เป็นตัวกลางสำหรับสร้างการเชื่อมต่อกับฐานข้อมูลและ
# ให้เมธอด CRUD สำหรับจัดการเมนู การจอง บิล และผู้ดูแลระบบ
...
@dataclass
class Menu:
    """ข้อมูลเมนูอาหาร (id, ชื่อ, ราคา, คำอธิบาย, รูป, หมวดหมู่)"""
    id: int
    name: str
    price: float
...
class DatabaseManager:
    ...
    def get_menus(self) -> List[Dict[str, Any]]:
        """คืนรายการเมนูทั้งหมดจากตาราง menus"""
    ...
    def add_menu(self, name: str, price: float, description: Optional[str], image: Optional[str], category: str):
        """เพิ่มเมนูใหม่ ในกรณีต้องการให้ ID ไม่ขาดตอน จะหา ID ว่างแล้วใช้ค่านั้น
        (สคริปต์ migrate_menus.py ใช้วิธีนี้ด้วย)"""
        with closing(self._connect()) as conn:
            # หา ID ว่างน้อยที่สุด
            cursor = conn.execute('SELECT id FROM menus ORDER BY id')
            existing_ids = {row[0] for row in cursor.fetchall()}
            
            # หา ID ว่างน้อยที่สุด (เริ่มจาก 1)
            new_id = 1
            while new_id in existing_ids:
                new_id += 1
            
            # INSERT ด้วย ID ที่กำหนดเอง
            conn.execute('INSERT INTO menus (id, name, price, description, image, category) VALUES (?, ?, ?, ?, ?, ?)',
                         (new_id, name, price, description, image, category))
            conn.commit()
    ...
    def get_reservations(self) -> List[Dict[str, Any]]:
        """ดึงการจองทั้งหมด (สำหรับหน้าแอดมินหรือรายงาน)"""
        with closing(self._connect()) as conn:
    ...
    def has_conflicting_reservation(self, date: str, table_no: int, start_time: str, end_time: str) -> bool:
        """ตรวจสอบว่ามีการจองซ้ำในช่วงเวลาที่ระบุหรือไม่
        ใช้สำหรับป้องกันการซ้อนทับของเวลาจองโต๊ะเดียวกัน
        """
        with closing(self._connect()) as conn:
    ...
    def add_reservation(self, name: str, phone: str, date: str, time: str, table_no: int, people: int):
        """บันทึกการจองใหม่ลงในตาราง reservations"""
        with closing(self._connect()) as conn:
    ...
    def get_user_reservations(self, phone: str) -> List[Dict[str, Any]]:
        """ดึงการจองทั้งหมดของผู้ใช้โดยใช้เบอร์โทรเป็นรหัส"""
        with closing(self._connect()) as conn:
            rows = conn.execute('SELECT * FROM reservations WHERE phone=? ORDER BY date DESC, time DESC', (phone,)).fetchall()
            return [dict(r) for r in rows]

    def get_reservation(self, reservation_id: int) -> Optional[Dict[str, Any]]:
        """ดึงข้อมูลการจองหนึ่งรายการตามรหัส"""
        with closing(self._connect()) as conn:
            row = conn.execute('SELECT * FROM reservations WHERE id=?', (reservation_id,)).fetchone()
            return dict(row) if row else None

    def update_reservation(self, reservation_id: int, date: str, time: str, people: int):
        """อัปเดตการจอง (เวลา, วันที่, และจำนวนคน)"""
        with closing(self._connect()) as conn:
            conn.execute('UPDATE reservations SET date=?, time=?, people=? WHERE id=?',
                         (date, time, people, reservation_id))
            conn.commit()
